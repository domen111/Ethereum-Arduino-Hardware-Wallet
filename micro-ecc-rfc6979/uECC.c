...
static int uECC_sign_with_k(const uint8_t *private_key,
    ...
    uint8_t reverse = 0;
    if (uECC_vli_cmp(s, secp256k1_half_n, num_n_words) > 0) {
        reverse = 1;
...
#endif    
    return (uECC_vli_testBit(p + num_words, 0) ^ reverse) | 2;
}

int rfc6979sha256p256sign(const uint8_t *private_key, const uint8_t *message_hash, uint8_t *signature)
{
    ...
    return uECC_sign_with_k(private_key, message_hash, 32, k, signature, uECC_secp256k1());
}
...
int uECC_sign(const uint8_t *private_key,
    ...
    for (tries = 0; tries < uECC_RNG_MAX_TRIES; ++tries) {
        ...
        int ret = 0;
        if (ret = uECC_sign_with_k(private_key, message_hash, hash_size, k, signature, curve)) {
            return ret;
        }
    }
    return 0;
